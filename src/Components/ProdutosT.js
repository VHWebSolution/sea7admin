...
const ProdutosT = ({ token }) => {
  ...
  const fetchItems = async () => {
    try {
      ...
      const sortedItems = response.data.sort((a, b) => b.id - a.id); // Ordena por ID decrescente
      const last5Items = sortedItems.slice(0, 5);
      setItems(last5Items);
    } catch (error) {
  ...
  const deletarProduto = async (itemId) => {
    ...
    await Swal.fire({
      ...
      if (result.isConfirmed) {
        const deleteUrl = `http://localhost:8080/deletar/${itemId}`;
